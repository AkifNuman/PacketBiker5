...
DisableAndroidFacebook(false), DisableAndroidApplovin(false), DisableIOSUnity(false), DisableIOSVungle(false), DisableIOSChartboost(false), DisableIOSFacebook(false),
DisableIOSApplovin(false), DisableIOSAdColony(false), DisableIOSIronSource(false), DisableIOSInMobi(false), DisableIOSTapJoy(false), DisableIOSIMobile(false)
{
...
void UAdmobSetting::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (EnableAndroidTestSuite)
	{
		...
		int curVersion = 0;
		if (GConfig->GetInt(AndroidSettings, TEXT("MinSDKVersion"), curVersion, GEngineIni))
		{
			if (curVersion < 19)
			{
				GConfig->SetInt(AndroidSettings, TEXT("MinSDKVersion"), 19, GEngineIni);
			}
		}
	}

	{
		FString IOSAddtionLinkFlags;
		if (GConfig->GetString(TEXT("/Script/IOSRuntimeSettings.IOSRuntimeSettings"), TEXT("AdditionalLinkerFlags"), IOSAddtionLinkFlags, GEngineIni)) {
			if (IOSAddtionLinkFlags.Find("-rpath /usr/lib/swift") == INDEX_NONE) {
				IOSAddtionLinkFlags += TEXT(" -rpath /usr/lib/swift");
				GConfig->SetString(TEXT("/Script/IOSRuntimeSettings.IOSRuntimeSettings"), TEXT("AdditionalLinkerFlags"), *IOSAddtionLinkFlags, GEngineIni);
			}
		}
	}
	...
	{
		FString IOSAddtionLinkFlags;
		if (GConfig->GetString(TEXT("/Script/IOSRuntimeSettings.IOSRuntimeSettings"), TEXT("AdditionalShippingLinkerFlags"), IOSAddtionLinkFlags, GEngineIni)) {
			if (IOSAddtionLinkFlags.Find("-rpath /usr/lib/swift") == INDEX_NONE) {
				IOSAddtionLinkFlags += TEXT(" -rpath /usr/lib/swift");
				GConfig->SetString(TEXT("/Script/IOSRuntimeSettings.IOSRuntimeSettings"), TEXT("AdditionalShippingLinkerFlags"), *IOSAddtionLinkFlags, GEngineIni);
			}
		}
	}
