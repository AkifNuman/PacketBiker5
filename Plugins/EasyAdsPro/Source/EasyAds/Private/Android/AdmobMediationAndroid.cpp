...
void AndroidLoadCustomBanner(const FString& adUnit)
{
	...
	{
		const bool bIsOptional = false;
		static jmethodID LoadCustomBannerMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_AdMob_LoadCustomBanner", "(Ljava/lang/String;)V", bIsOptional);
		if (LoadCustomBannerMethod == nullptr)
		{
			UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_AdMob_LoadCustomBanner not found"));
			return;
		}
		auto adUnitJava = FJavaHelper::ToJavaString(Env, adUnit);
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, LoadCustomBannerMethod, *adUnitJava);
	}
}
...
void AndroidShowCustomBanner(bool isBottom)
{
	...
	{
		const bool bIsOptional = false;
		static jmethodID ShowBannerMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_AdMob_ShowCustomBanner", "(Z)V", bIsOptional);
		if (ShowBannerMethod == nullptr)
		{
			UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_AdMob_ShowCustomBanner not found"));
			return;
		}
...
void AndroidHideCustomBanner()
{
	...
	{
		const bool bIsOptional = false;
		static jmethodID HideBannerMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_AdMob_HideCustomBanner", "()V", bIsOptional);

		if (HideBannerMethod == nullptr)
		{
			UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_AdMob_HideCustomBanner not found"));
			return;
		}
...
void AndroidSetTagForChildDirectedTreatment(int setting)
{
	...
	{
		const bool bIsOptional = false;
		static jmethodID SetTagForChildDirectedTreatmentMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_AdMob_SetTagForChildDirectedTreatment", "(I)V", bIsOptional);
		if (SetTagForChildDirectedTreatmentMethod == nullptr)
		{
			UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_AdMob_SetTagForChildDirectedTreatment not found"));
			return;
		}
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, SetTagForChildDirectedTreatmentMethod, setting);
	}
}
...
void AndroidSetTagForUnderAgeOfConsent(int setting)
{
	...
	{
		const bool bIsOptional = false;
		static jmethodID SetTagForUnderAgeOfConsentMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_AdMob_SetTagForUnderAgeOfConsent", "(I)V", bIsOptional);
		if (SetTagForUnderAgeOfConsentMethod == nullptr)
		{
			UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_AdMob_SetTagForUnderAgeOfConsent not found"));
			return;
		}
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, SetTagForUnderAgeOfConsentMethod, setting);
	}
}
...
void AndroidSetMaxAdContentRating(int setting)
{
	...
	{
		const bool bIsOptional = false;
		static jmethodID SetMaxAdContentRatingMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_AdMob_SetMaxAdContentRating", "(I)V", bIsOptional);
		if (SetMaxAdContentRatingMethod == nullptr)
		{
			UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_AdMob_SetMaxAdContentRating not found"));
			return;
		}
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, SetMaxAdContentRatingMethod, setting);
	}
}
...
bool AndroidIsAdNetworkConsent(AdNetworks adnetwork) {
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		...
		if (adnetwork == AdNetworks::Admob) {
			static jmethodID checkMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_IsAdmobConsent", "()Z", bIsOptional);
			if (checkMethod == nullptr)
			{
				UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_IsAdmobConsent not found"));
				return false;
			}
			return FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, checkMethod);
		}
		else {
			static jmethodID checkMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_IsMediationConsent", "(I)Z", bIsOptional);
			if (checkMethod == nullptr)
			{
				UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_IsAdmobConsent not found"));
				return false;
			...
			return FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, checkMethod, vendor_id);
		}
	}
...
void AndroidUMPRequestConsentInfoUpdate(bool underAgeOfConsent)
{
	...
	{
		const bool bIsOptional = false;
		static jmethodID RequestConsentMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_AdMob_UMPRequestConsentInfoUpdate", "(Z)V", bIsOptional);
		if (RequestConsentMethod == nullptr)
		{
			UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_AdMob_UMPRequestConsentInfoUpdate not found"));
			return;
		}
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, RequestConsentMethod, underAgeOfConsent);
	}
}
...
void AndroidUMPResetConsent()
{
	...
	{
		const bool bIsOptional = false;
		static jmethodID resetConsentMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_AdMob_UMPResetConsent", "()V", bIsOptional);
		if (resetConsentMethod == nullptr)
		{
			UE_LOG(EasyAds, Error, TEXT("AndroidThunkJava_AdMob_UMPResetConsent not found"));
			return;
		}
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, resetConsentMethod);
	}
}
...
__attribute__((visibility("default"))) extern "C" void Java_com_ads_util_AdMob_nativeCustomBannerLoad(JNIEnv * jenv, jobject thiz, jboolean success, jstring errmsg)
{
	FString strErrorMessage = FJavaHelper::FStringFromParam(jenv, errmsg);

	DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.Java_com_ads_util_AdMob_nativeCustomBannerLoad"), STAT_FSimpleDelegateGraphTask_Java_com_ads_util_AdMob_nativeCustomBannerLoad, STATGROUP_TaskGraphTasks);
	FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
		FSimpleDelegateGraphTask::FDelegate::CreateLambda([=]()
			{
				FPlatformMisc::LowLevelOutputDebugStringf(TEXT("nativeCustomBannerLoad\n"));
				if (g_CustomBannerAd)
				{
					if (success) {
						...
						g_CustomBannerAd->OnLoadFailCallback(strErrorMessage);
					}
					
				}
			}),
		GET_STATID(STAT_FSimpleDelegateGraphTask_Java_com_ads_util_AdMob_nativeCustomBannerLoad),
				nullptr,
				ENamedThreads::GameThread
				);
}
...
__attribute__((visibility("default"))) extern "C" void Java_com_ads_util_AdMob_nativeOnUMPConsentResult(JNIEnv* jenv, jobject thiz, jboolean isSuccess, jstring err)
{
	DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.Java_com_ads_util_AdMob_nativeOnUMPConsentResult"), STAT_FSimpleDelegateGraphTask_Java_com_ads_util_AdMob_nativeOnUMPConsentResult, STATGROUP_TaskGraphTasks);
	
	FString strErr = FJavaHelper::FStringFromParam(jenv, err);
	FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
		FSimpleDelegateGraphTask::FDelegate::CreateLambda([=]()
			{
				FPlatformMisc::LowLevelOutputDebugStringf(TEXT("nativeOnUMPConsentResult;%d\n"), (int)isSuccess);
				UUMPRequestConsentInfoUpdate::OnConsentUpdateCallback(isSuccess, strErr);
			}),
		GET_STATID(STAT_FSimpleDelegateGraphTask_Java_com_ads_util_AdMob_nativeOnUMPConsentResult),
				nullptr,
				ENamedThreads::GameThread
				);
}
